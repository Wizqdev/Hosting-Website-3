...
app.get("/vps", (req, res) => {
    res.render("vps.ejs", {config: config});
  });

app.get("/game-servers", (req, res) => {
    res.render("game-servers.ejs", {config: config});
  });

app.get("/dedicated", (req, res) => {
    res.render("dedicated.ejs", {config: config});
  });
...
app.listen(port, () => {
    console.log(chalk.blue(chalk.bold(`System`)), (chalk.white(`>>`)), chalk.red(`Website Ruing On`), chalk.green(`${port}`))                                                                                  
});